...
export const useAtmDataCtx = () => useContext(AtmData);

const AtmDataProvider = ({
  children,
  atms,
}: PropsWithChildren<{
  atms: ATMWithCoordinates[];
}>) => {
  const validATMs = useMemo(
    () =>
      atms.filter(
        (atm) =>
          atm.latitude &&
  ...
  return (
    <AtmData.Provider value={{ atms: validATMs }}>{children}</AtmData.Provider>
  );
};
      ...
      <Popup>
        <div className="flex flex-col">
          ...
          <p className="text-sm text-gray-600">
            Đây là vị trí hiện tại của bạn
          </p>
        </div>
      </Popup>
    </Marker>
